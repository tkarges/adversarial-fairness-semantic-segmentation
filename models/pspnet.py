"""
PSPNet and DeepLabV3 model classes adapted from the implementation accompanying:

Xiaogang Xu, Hengshuang Zhao, and Jiaya Jia,
"Dynamic Divide-and-Conquer Adversarial Training for Robust Semantic Segmentation"
ICCV, 2021.

DDC-AT source:
https://github.com/JIA-Lab-research/Robust-Semantic-Segmentation/blob/main/model/pspnet.py

The DDC-AT implementation is itself based on this implementation:
https://github.com/hszhao/semseg

The hszhao/semseg repository is distributed under the MIT License.

This file contains minor modifications for this project, mainly removal of
implementation-specific artifacts such as local file paths.
"""
import torch
...
class DeepLabV3(nn.Module):
    ...
    def forward(self, x, y=None, indicate=0, return_aux=False):
        '''
        x_size = x.size()
        ...
        if (self.training or indicate == 1) and y is not None:
            aux = self.aux(x_tmp)
            if self.zoom_factor != 1:
                aux = F.interpolate(aux, size=(h, w), mode='bilinear', align_corners=True)
            main_loss = self.criterion(logits, y)
            aux_loss = self.criterion(aux, y)
            return logits.max(1)[1], main_loss, aux_loss, logits
...
class PSPNet(nn.Module):
    ...
    def forward(self, x, y=None, indicate=0, return_aux=False):
        ...
        # Slight modifications to the return logic compared to the original:
        # The model should simply return x, since that fits the training and attack code
        if (self.training or indicate==1) and y is not None:
            aux = self.aux(x_tmp)
            if self.zoom_factor != 1:
                aux = F.interpolate(aux, size=(h, w), mode='bilinear', align_corners=True)
